...
def compute(string):
	replace = (("[", "("), ("]", ")"), ("{", "("), ("}", ")"), ("^", "**"))
	string = compile_parenthesis(string, latex=False)
...
def compile_parenthesis(line, latex=False):
	global MODE
	...
	if MODE == MATH:
		...
		while i < len(string):
			if string[i] == ")":
				string = string[:prev] + compile_expression(string[prev+1:i], latex=latex) + string[i+1:]
				i, prev = 0, 0
			elif string[i] == "(":
				prev = i
			i += 1
...
def compile_line(line, latex=True):
	...
	line = compile_parenthesis(line, latex=latex)
	...
	if not latex:
		...
		if line.startswith( "\\def"):
			...
			arguments = line[line.find("{")+1:line.find("}")].replace(" ", "").split(",")
